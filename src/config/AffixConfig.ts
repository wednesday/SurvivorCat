import { DIFFICULTY_CONFIGS, DifficultyLevel, DifficultySettings } from "./DifficultyConfig";
...
export const AFFIX_TEMPLATES: AffixTemplate[] = [
  ...
  // ========== 特殊词条 ==========
  {
    ...
    minQuality: Rarity.Epic,
  },
];

export function getAffixTemplateById(
...
// 随机生成装备品质
export function rollEquipmentQuality(
  difficultyLevel: DifficultyLevel
): Rarity {
  const difficultySettings: DifficultySettings = DIFFICULTY_CONFIGS[difficultyLevel];
  const rand = Math.random();
  // 通过 difficultySettings.rarityDropRateMultiplier[0-10] 来调整掉落概率,当接近10时，Legendary概率为极限50%
  const adjustedLegendaryRate = Math.min(
    0.02 + (difficultySettings.rarityDropRateMultiplier - 1) * 0.048,
    0.5
  );
  const adjustedEpicRate = Math.min(
    0.08 + (difficultySettings.rarityDropRateMultiplier - 1) * 0.12,
    0.4
  );
  const adjustedRareRate = Math.min(
    0.15 + (difficultySettings.rarityDropRateMultiplier - 1) * 0.15,
    0.3
  );

  if (rand < adjustedLegendaryRate) return Rarity.Legendary; // e.g., up to 50%
  if (rand < adjustedLegendaryRate + adjustedEpicRate) return Rarity.Epic; // e.g., up to 40%
  if (rand < adjustedLegendaryRate + adjustedEpicRate + adjustedRareRate)
    return Rarity.Rare; // e.g., up to 30%
  return Rarity.Common; // rest
}
...
export function generateEquipmentName(
  baseName: string,
  ...
  let prefixAffix = affixes.find((a) => {
    ...
    return (
      tpl && (tpl.type === AffixType.Special || tpl.type === AffixType.Effect)
    );
  });

  // 如果没有特殊词条，选择稀有度最高的词条
  if (!prefixAffix) {
    prefixAffix = affixes.reduce(
      (prev, curr) => (curr.rarity > prev.rarity ? curr : prev),
      affixes[0]
    );
  }
...
export function rollAffixes(
  ...
  const allowedTiers = RARITY_TIERS.filter((tier) => {
    if (config.maxRarity) {
      const maxIndex = RARITY_TIERS.findIndex(
        (t) => t.rarity === config.maxRarity
      );
      const tierIndex = RARITY_TIERS.findIndex((t) => t.rarity === tier.rarity);
      if (tierIndex > maxIndex) return false;
    }
    if (config.minRarity) {
      const minIndex = RARITY_TIERS.findIndex(
        (t) => t.rarity === config.minRarity
      );
      const tierIndex = RARITY_TIERS.findIndex((t) => t.rarity === tier.rarity);
  ...
  const totalTemplateWeight = () =>
    pool.reduce((s, p) => s + (p.weight || 1), 0);
